...
**Verifiziert von ThinkPad X1 (2026-08-11):** Nicht nur die Aktivierung fehlt — `.obsidian/plugins/` existiert auf X1 für `Unterwegs` **komplett nicht** (kein einziger Ordner, keine `community-plugins.json`). Zum Vergleich: `Touren` hat auf demselben Gerät beide (`plugins/`-Ordner + `community-plugins.json`). Der Plugin-Code aus dieser 203-Migration ist also noch nicht über Obsidian Sync auf X1 angekommen — vermutlich fehlt entweder der Sync-Toggle „Installierte Community-Plugins" für den neuen Vault, oder der Sync-Lauf hat den nachträglich (nach der Vault-Ersteinrichtung) kopierten Plugin-Code noch nicht nachgezogen. Die Sync-interne Toggle-Einstellung selbst ist nicht als Klartext-Datei im Vault gespeichert und daher von außerhalb der App nicht direkt prüfbar. **Betroffen sind aktiv genutzte Inhalte:** `obsidian-map-view`-Einbettungen in `Ressourcen/GPX-Setup-Anleitung.md`, `Ressourcen/GPX-Verwaltung.md`, `Stadtrundgänge/README.md`, `Stadtrundgänge/Template - Stadtrundgang.md`, `Wanderungen/Template - Wanderung.md` rendern auf X1 aktuell nicht. **Nächster Schritt (User):** In Obsidian bei geöffnetem Vault unter Einstellungen → Sync prüfen, ob „Installierte Community-Plugins" aktiviert ist; danach Plugins unter Einstellungen → Community-Plugins einzeln aktivieren.

**Tatsächliche Root Cause gefunden (2026-08-11):** Der Sync-Toggle „Installierte externe Erweiterungen" war auf X1 bereits aktiviert (User-Screenshot) — half aber nicht, weil das Problem auf der Server-Seite liegt. `ob sync-status --path .../Unterwegs` zeigt auf **beiden** Headless-Clients (201 `pvebs-nc-vault-sync`, 203 `pvebs-nc-claude-worker`) identisch: `Configs: app, appearance, appearance-data, hotkey, core-plugin, core-plugin-data, community-plugin-data` — **`community-plugin` fehlt** (das ist exakt die Kategorie für die Plugin-**Binärdateien** `.js`/`.css`/`manifest.json`; `community-plugin-data` synct nur `data.json`-Einstellungen). Container 203 hat die 11 Plugin-Ordner zwar lokal auf der Platte (per Datei-Kopie aus `Urlaub`/`Touren` angelegt), pusht sie aber **nie** zu Obsidian Sync, weil diese Kategorie in seiner eigenen `ob sync-config` nicht eingeschlossen ist — der Cloud-Snapshot hat die Dateien folglich nie erhalten, egal welchen Toggle X1 clientseitig setzt. **Zum Vergleich:** Dieselbe fehlende Kategorie besteht auch bei `Touren` auf 201/203 — dort ist es unschädlich, weil die Plugin-Dateien für `Touren` schon vor der Headless-Sync-Einrichtung direkt über X1s eigenen Sync-Client hochgeladen wurden. Bei `Unterwegs` (komplett neu, Plugin-Code nur auf 203 lokal kopiert) greift dieser Umweg nicht. **Fix (noch nicht ausgeführt, braucht Freigabe):** `ob sync-config --path /home/claude/Obsidian-Vaults/Unterwegs --configs app,appearance,appearance-data,hotkey,core-plugin,core-plugin-data,community-plugin,community-plugin-data` auf 203 (Quelle der lokalen Dateien), danach `ob sync` anstoßen; optional dieselbe Korrektur auf 201 für zukünftige Fälle.
